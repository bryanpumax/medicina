...
$nombreFile=$nom.'Informe_'.$fechai.'.xlsx';
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
$objWriter->save(str_replace('diario.php',$nombreFile , __FILE__));
$callEndTime = microtime(true);
$callTime = $callEndTime - $callStartTime;

$idx_diario='';
$consulta=$pdo->query("SELECT * FROM tbl_diario WHERE fec_informe='$fecha' and idx_operador='$idx_operador'");
$row=$consulta->fetch();
...
if(is_null($idx_diario)){
  ...
  $idx_diario=GeneraCodigo($nom.'INF','tbl_parametros','n_informe');
  $idactualziado=ActualizaCodigo('tbl_parametros','n_informe');
}
$borra=$pdo->query("DELETE FROM tbl_diario WHERE fec_informe='$fecha' and idx_operador='$idx_operador'");
