...
<div class="">
<div class="col-xs-12 col-md-10 col-md-offset-1  col-sm-10 col-sm-offset-1   col-lg-10 col-lg-offset-1">
<div class="box">
<header style="background:#ff6a29!important;color:#000 ">
<div class="icons"> <i class="fas fa-address-book"></i>
</div>
<h5>Citas establecidads para el :&nbsp;</h5>
 <h5 >'.$fecha.'</h5>
...
<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped table-xs  ">
    <thead>
...
echo '<td  ><div style="color: blue;">' . ($tipo['hora_citas']) . '</div></td> ';}
  ...
  echo '<td style="color: magenta;" >' . ($tipo['hora_citas']) . '</td> ';}
  if($tipo['estado_citas']=="cumplida"){
    echo '<td  style="color: green;" class="mayuscula btn-xs">' . ($tipo['cliente']) . '</td>'; 
...
echo '<td align="justify"><a href="index2.php?gettis=cumplida&id_citas='.$tipo['id_citas'].'" class=""><button title=" Visita Cumplida" class="btn btn-success btn-xs">
<i class="fa-2x fas fa-check-square"></i></button></a>

<a href="index2.php?gettis=cancelar&id_citas='.$tipo['id_citas'].'" class=""><button class="btn btn-danger btn-xs" title="Cancelar Visita">
<i class="fa-2x fas fa-window-close"></i></button></a>

<a href="index2.php?gettis=regendar&id_citas='.$tipo['id_citas'].'" class=""><button class="btn btn-info btn-xs" title="Regeandar Visitar">
<i class="fa-2x fas fa-calendar-alt"></i>
 </button></a>
 <a href="index2.php?gettis=opinion&id_citas='.$tipo['id_citas'].'" class=""><button class="btn btn-warning btn-xs" title="Observaciones de la Visita">
...
<div class="">
<div class="box">
<header style="background:#1cd9ff!important;">
<div class="icons"><i class="fas fa-briefcase"></i></div>
<h5>Raking de productos mas vendidos</h5>
</header>


<div id="collapse4" class="body ">

<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped  table-xs sortableTable">
    <thead>
    ...
    while ($tipo = mysqli_fetch_array($res)) {
$porcentaje=(($tipo['contar_inventario']*5)/100)*100;
$valor=$tipo['contar_inventario'];

if($contar==1) $fondo='alert alert-secondary';

if($contar==2) $fondo='danger';

if($contar==3) $fondo='alert alert-warning bg-warning text-white border-0';

if($contar==4) $fondo='alert alert-success bg-success text-white border-0 ';

if($contar==5) $fondo='alert alert-info bg-info text-white border-0 ';
$contar++;
 ...
 echo '<td>
    ...
    <div class="progress-bar progress-bar-info" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100"
         style="width:'.$porcentaje.'" title="'.$porcentaje.'%">
      <span >'.$valor.' completado</span>
    </div>
  </div>
...
<h5>Raking de empresas</h5>
</header>


<div id="collapse4" class="body ">

<table id="dataTable" class="table table-bordered table-condensed table-hovered table-striped table-xs sortableTable ">
    <thead>
...
$contar=1;
    while ($tipo = mysqli_fetch_array($res)) {
if($contar==1) $fondo='active';

if($contar==2) $fondo='danger';

if($contar==3) $fondo='warning';

if($contar==4) $fondo='success';

if($contar==5) $fondo='danger';
$contar++;
  ...
  echo '<td   >' . ($tipo['emp_clientes']) . '</td> ';
    ...
    echo '<td  class="mayuscula badge alert-warning text-center">' . ($tipo['contar']) . '</td>
 </tr>';
