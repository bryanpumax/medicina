...
$hoy  =   date("d-m-Y")  ;
    ...
    $resultr = mysqli_query($conexion, $sql);
...
<div id="left">
    ...
    <ul id="menu" class="  ">
      ...
      <!--menu desplegable-->
      <li>
      <a ><i class="far fa-folder-open fa-border"></i><span class="link-title">&nbsp;ARCHIVO</span>  <span class="fa arrow"></span>  </a>
      <ul class="collapse">';
        
    while ($item = mysqli_fetch_array($result)) {
        $titulo = strtoupper($item['etiq_menu']);
        $ico = $item['icono_menu'];
        $ruta=$item['ruta_menu'];
     if($item['jerarquia']=="a"){
        $contenido .= '
        <li  >
<a onclick="scrollWin()" target="iframe" href="'.$ruta.'"><i class="'.$ico.'"></i><span class="link-title">&nbsp;'.$titulo.'</span></a>
    </li>';
    }
}
      $contenido.='</ul></li>
      <!--menu desplegable-->
      <li>
      <a   ><i class="fas fa-arrows-alt fa-border"></i><span class="link-title">&nbsp;MOVIMIENTO</span>  <span class="fa arrow"></span>  </a>
      <ul class="collapse">';

      while ($item = mysqli_fetch_array($resultm)) {
        $titulo = strtoupper($item['etiq_menu']);
        $ico = $item['icono_menu'];
        $ruta=$item['ruta_menu'];
     if($item['jerarquia']=='m'){
...
<a onclick="scrollWin()" target="iframe" href="'.$ruta.'"><i class="'.$ico.'"></i><span class="link-title">&nbsp;'.$titulo.'</span></a>
      ...
      <!--menu desplegable-->
      <li>
      <a  ><i class="fas fa-book-reader fa-border"></i><span class="link-title">&nbsp;REPORTE</span>  <span class="fa arrow"></span>  </a>
      <ul class="collapse">';
      while ($item = mysqli_fetch_array($resultr)) {
        $titulo = strtoupper($item['etiq_menu']);
        $ico = $item['icono_menu'];
        $ruta=$item['ruta_menu'];
     if($item['jerarquia']=='r'){
...
<a onclick="scrollWin()" target="iframe" href="'.$ruta.'"><i class="'.$ico.'"></i><span class="link-title">&nbsp;'.$titulo.'</span></a>
      ...
      <li><a data-placement="bottom" data-original-title="Ver / Ocultar" data-toggle="tooltip" class="btn btn-primary btn-sm toggle-left" id="menu-toggle">
...
function footer($año,$autor){
  echo '<footer class="Footer bg-dark dker">
    <p>Diseñado por Victor Manuel Cardenas.'.$año.' by @ &copy;'.$autor.'</p>
</footer>';
