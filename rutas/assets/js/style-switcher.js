...
StyleSwitcher.prototype.build = function () {
    ...
    var h5 = $('<h5 />').html('Cambiar el estilo de menu').append(h5A);
    ...
    var styleSwitcherWrap = $('<div />')
        ...
        .append($('<h6 />').html('Color para  el fondo del menu'), colorList, $('<hr/>'));
    ...
    var fontBlack = $('<label/>').addClass('btn btn-xs btn-danger text-dark').html('Negro').append(fgbbtn);
    var fgBtnGroup = $('<div/>').addClass('btn-group').attr('data-toggle', 'buttons').append(fontWhite, fontBlack);
    styleSwitcherWrap.append($('<div/>').addClass('options-link').append($('<h6/>').html('Color de texto'), fgBtnGroup));
    ...
    var resetLink = $('<a/>').html('Restablecer').attr('href', '#').on(Metis.buttonPressedEvent, function (e) {
        $this.reset();
        e.preventDefault();
    });
